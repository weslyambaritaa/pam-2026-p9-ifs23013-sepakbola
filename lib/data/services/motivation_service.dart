// lib/data/services/motivation_service.dart
import 'dart:convert';
...
class MotivationService {
        ...
        static Future<void> generateMotivation(String theme, int total) async {
                ...
                if (response.statusCode != 200) {
                        // Tangkap pesan error dari JSON backend (misal: "Total maksimal harus 10")
                        try {
                                final errorData = jsonDecode(response.body);
                                throw Exception(errorData["error"] ?? "Gagal melakukan generate");
                        } catch (_) {
                                throw Exception("Gagal melakukan generate (Status: ${response.statusCode})");
                        }
                }
        }
}